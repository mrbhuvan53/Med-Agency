...
-- Add this new table for user registration and login
CREATE TABLE `users` (
  ...
  `medical_shop_name` varchar(200) NOT NULL,
  `address` varchar(300) NOT NULL,
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Modify the posts table to include a foreign key to users
ALTER TABLE `posts`
ADD COLUMN `user_id` int(11),
ADD CONSTRAINT `fk_user_posts` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`);

-- Modify the medicines table to include a foreign key to users
ALTER TABLE `medicines`
ADD COLUMN `user_id` int(11),
ADD CONSTRAINT `fk_user_medicines` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`);
...
(1001, 'Bhuvan', 'ANEES', '7896541230', 'Bangalore');
